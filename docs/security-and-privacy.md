...
The alpha has one narrowly bounded external-write capability: `workctx outbox send`. It posts one canonical draft body to one explicitly named GitHub issue or pull request. Preview is the default; one send requires `--yes`, an exact fingerprint of the reviewed draft content plus channel and target, and — in JSON mode — the script must echo that fingerprint. The body is secret-scanned again at the last gate, a sent draft refuses revision or resend, and successful delivery is recorded with remote comment provenance through one approved hash-chained ledger transaction. There is no batch, send-all, scheduling, retry, MCP send, other channel, attachment, or implicit reuse of draft-save approval ([outbox send reference](reference/outbox-send.md)).

A delivery failure leaves canonical draft and ledger state unchanged and returns a content-free diagnostic. GitHub and the local filesystem cannot form one distributed transaction: if the remote comment succeeds but the local provenance transaction returns a failure, workctx reports reconciliation-required remote ID/URL metadata and refuses to describe the operation as a clean failure. A process stop in that interval can leave no local receipt at all. In either case, the operator must verify the target and must not resend blindly. This is the alpha's narrower external-write guarantee, not a claim of cross-system atomicity.
