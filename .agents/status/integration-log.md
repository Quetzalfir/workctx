...
| 2026-09-23 | WP-830-secret-acknowledgment | capture | merge | worker gate partial (sandbox ACL, uv cache denied); lead full gate: ruff+format+mypy 0, pytest 1772 passed (1 blocker round: ledger/domain serialization split caused 142 failures — unified via model serializer) | accepted+integrated (1 scope amendment) | C-222; operator-found via Loftware agent CTX-POSSIBLE-SECRET block |
| 2026-09-24 | WP-840-outbound-voice | capture | merge | worker full suite ran (1774 passed, lead-owned reconciliations); lead gate: ruff+format+mypy 0, pytest 1778 passed | accepted+integrated | C-223; first-person voice + conversation-language detection in skill, bridges, template |
